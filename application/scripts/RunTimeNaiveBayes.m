...
timeNaiveBayesModel = TimeNaiveBayesModel(graphHandler, nodes, featuresHandler);
...
[features, labels, timestamps] = dataLoader.LoadMultipleSensorsData(validationData('0').test);
...
labelsIndex = graphHandler.GetLabelsIndex() + length(graphHandler.AllNodesNames) * 2;

sumResults = zeros(samplesCount, 4);

for s=3:samplesCount
    if (mod(s, 100) == 0)
        disp(['Running test sample ', num2str(s), '/', num2str(samplesCount)]);
    end
    
    evidence = timeNaiveBayesModel.RawDataToTestTimeGraphData(features(s-2:s, :), labels(s-2:s, :));
...
% Last successful run - Total score for the test run: 0.5